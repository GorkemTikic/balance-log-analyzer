...
import {
    buildSummaryRows, composeNarrative, parseUTC,
    parseBaseline, parseTransfer, parseFinalBalancesFromAudit
} from "@/lib/story";
...
export default function StoryNarrative({ rows, lang, inputs, setters }: StoryTabProps) {
    ...
    async function copyStory() {
        try { await navigator.clipboard.writeText(friendlyText); alert("Copied."); }
        ...
        <div style={{ marginTop: 16 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16, alignItems: "start" }}>
                <label className="text-muted" style={{ minWidth: 0 }}>
                    {T.startTime}
                    <input className="input-block" value={start} onChange={(e) => setStart(e.target.value)} placeholder="YYYY-MM-DD HH:MM:SS" />
                </label>
                <label className="text-muted" style={{ minWidth: 0 }}>
                    {T.baseline}
                    <textarea className="input-block" style={{ minHeight: 64, fontFamily: "monospace", fontSize: 13 }}
                        placeholder={`USDT 1000\n0.5 BTC`} value={baselineText} onChange={(e) => setBaselineText(e.target.value)} />
                </label>
                <div style={{ minWidth: 0 }}>
                    <div className="text-muted">{T.transferAtStart}</div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 6 }}>
                        <input className="input-block" placeholder={T.amount} value={trAmount} onChange={(e) => setTrAmount(e.target.value)} />
                        <input className="input-block" placeholder={T.asset} value={trAsset} onChange={(e) => setTrAsset(e.target.value)} />
                    </div>
                </div>
            </div>
            ...
            <div className="card" style={{ marginTop: 16, padding: 16 }}>
                <div className="flex-between" style={{ marginBottom: 12 }}>
                    <h4 className="section-title" style={{ margin: 0 }}>{T.narrative}</h4>
                    <button className="btn small" onClick={copyStory}>{T.copyStory}</button>
                </div>
                <pre className="mono" style={{ whiteSpace: "pre-wrap", fontSize: 13, background: "rgba(0,0,0,0.3)", padding: 16, borderRadius: 8 }}>
                    {friendlyText}
                </pre>
            </div>
            ...
            <div className="card" style={{ marginTop: 16 }}>
                <div className="section-head flex-between">
                    ...
                    <button className="btn small" onClick={exportSummaryPng}>{T.exportPng}</button>
                </div>
                        ...
                        <tbody>
                            {summaryRows.map((r, i) => (
                                <tr key={i}>
                                    ...
                                    <td className="text-left"><AssetIcon asset={r.asset} /> {r.asset}</td>
                                    <td className={`text-right ${r.in ? "text-green" : "text-muted"}`}>{r.in ? `+${fmtTrim(r.in)}` : "—"}</td>
                                    <td className={`text-right ${r.out ? "text-red" : "text-muted"}`}>{r.out ? `-${fmtTrim(r.out)}` : "—"}</td>
...
function AssetIcon({ asset }: { asset: string }) {
    const map: Record<string, string> = {
        BTC: "🟧", ETH: "⚪", BNB: "🟡", USDT: "🟩", USDC: "🔵", BFUSD: "🟦", FDUSD: "🟪", LDUSDT: "🟩", BNFCR: "🟠"
    };
