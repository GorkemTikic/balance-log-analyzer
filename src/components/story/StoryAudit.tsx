...
export default function StoryAudit({ rows, lang, inputs, setters }: StoryTabProps) {
    ...
    async function copyAudit() {
        try { await navigator.clipboard.writeText(auditText); alert("Copied."); }
    ...
    return (
        <div style={{ marginTop: 16 }}>
            <div className="flex-between" style={{ marginBottom: 12 }}>
                <h4 className="section-title" style={{ margin: 0 }}>{T.agentAudit}</h4>
                <button className="btn small" onClick={copyAudit}>{T.copyAudit}</button>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px,1fr))", gap: 16, alignItems: "start" }}>
                ...
                <label className="text-muted">
                    {T.endTime}
                    <input className="input-block" value={end} onChange={(e) => setEnd(e.target.value)} placeholder="YYYY-MM-DD HH:MM:SS" />
                </label>
            </div>
            ...
            <div style={{ marginTop: 16, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <label className="text-muted">
                    {T.baseline}
                    <textarea className="input-block" style={{ minHeight: 120, fontFamily: "monospace", fontSize: 13 }}
                        placeholder={`One per line:\nUSDT 1000`} value={baselineText} onChange={(e) => setBaselineText(e.target.value)} />
                </label>
                <div>
            ...
            <div className="card" style={{ marginTop: 16, padding: 12 }}>
                ...
                <pre className="mono" style={{ whiteSpace: "pre-wrap", fontSize: 13, background: "rgba(0,0,0,0.3)", padding: 12, borderRadius: 8, maxHeight: 480, overflow: "auto" }}>
                    {auditText}
                </pre>
            </div>
        </div>
