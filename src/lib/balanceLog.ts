...
export function textToGrid(text: string): { grid: RawTable; format: ParseResult["inputFormat"] } {
  ...
  // whitespace fallback
  return {
    grid: lines.map((l) => l.trim().split(/\s{2,}|\s\|\s|\s+/).map((c) => c.trim())),
    format: "whitespace"
  };
}
...
export function parseGrid(grid: RawTable, format: ParseResult["inputFormat"] = "table"): ParseResult {
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    const decision = decideScope({ asset: parsed.asset, type: parsed.type, symbol: parsed.symbol, raw: parsed.raw });
    if (decision.scope === "USDM" || decision.scope === "UNKNOWN") {
      result.included.push(parsed);
      if (decision.scope === "UNKNOWN") {
        result.warnings.push({ rowIndex: parsed.rowIndex, message: decision.reason });
      }
    } else {
      result.excluded.push({ ...parsed, excludeReason: decision.reason });
    }
  }
  return result;
}
