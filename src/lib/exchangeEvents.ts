...
function buildSummary(kind: ExchangeKind, time: string, outLegs: ExchangeLeg[], inLegs: ExchangeLeg[], rateText: string, pegNote: string) {
...
export function buildExchangeEvents(rows: Row[], kind: ExchangeKind): ExchangeEvent[] {
  ...
  for (const [id, group] of groups.entries()) {
    ...
    events.push({
      ...
      pegNote: peg.pegNote,
      summary: buildSummary(kind, time, outLegs, inLegs, rateText, peg.pegNote)
    });
  }
